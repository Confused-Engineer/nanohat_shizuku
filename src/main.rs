...
impl NanoPi
{
    ...
    fn start(&mut self)
    {
        ...
        if let Err(image) = self.screen.draw_image(include_bytes!("../assets/screen_main.raw"), 120)
        {
            eprintln!("{}", image)
        }
            ...
            match self.state {
                AppState::Main(menu) => {
                    ...
                    if self.screen_refresh_required
                    {
                        ...
                        if let Ok(_) = self.screen.draw_image(include_bytes!("../assets/screen_main.raw"), 0)
                        {
